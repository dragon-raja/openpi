...
class LeRobotRoboCasaDataConfig(DataConfigFactory):
    """Data config for RoboCasa datasets converted to LeRobot format."""
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        repack_transform = _transforms.Group(
            inputs=[
                _transforms.RepackTransform(
                    {
                        "observation/base_image": "observation.images.robot0_agentview_left",
                        "observation/left_wrist_image": "observation.images.robot0_eye_in_hand",
                        "observation/right_wrist_image": "observation.images.robot0_agentview_right",
...
class TrainConfig:
    ...
    physical_prompt_rank_warmup_steps: int = 0
    ...
    def __post_init__(self) -> None:
        ...
        if self.physical_prompt_rank_warmup_steps < 0 or self.physical_prompt_rank_ramp_steps < 0:
...
_CONFIGS = [
    ...
    TrainConfig(
        name="pi05_libero_causal_icl_curriculum_lora",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,
        ...
        physical_prompt_counterfactual_rank_weight=0.15,
        physical_prompt_action_rank_weight=0.15,
        physical_prompt_rank_margin=0.10,
        physical_prompt_rank_warmup_steps=150,
        physical_prompt_rank_ramp_steps=149,
        physical_prompt_rank_normalize=True,
        physical_prompt_rank_scale_floor=0.05,
        ema_decay=None,
    ),
    # C2.1 is intentionally hyperparameter-identical to C2.  Its separate
    # name keeps the corrected one-step action-effect run from overwriting the
    # sparse-interval C2 negative control.
    TrainConfig(
        name="pi05_libero_causal_icl_aligned_effect_lora",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,
    ...
    TrainConfig(
        name="pi05_libero_directed_behavior_binding_lora",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,
    ...
    TrainConfig(
        name="pi05_libero_visually_routed_behavior_binding_lora",
        model=pi0_config.Pi0Config(
            pi05=True,
            action_horizon=10,
            discrete_state_input=False,
    ...
    TrainConfig(
        ...
        data=LeRobotRoboCasaDataConfig(
            repo_id="robocasa/ArrangeBreadBasket",
            base_config=DataConfig(prompt_from_task=True),
        ),
        batch_size=8,
        lr_schedule=_optimizer.CosineDecaySchedule(
